...
def x2conv(in_channels, out_channels, inner_channels=None):
    ...
    down_conv = nn.Sequential(
        nn.Conv2d(in_channels, inner_channels, kernel_size=3, padding=1, bias=False), #3x3 的卷积 通道数不变
        nn.BatchNorm2d(inner_channels),
        nn.ReLU(inplace=True),
        nn.Conv2d(inner_channels, out_channels, kernel_size=3, padding=1, bias=False), #通道数扩张
...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.pool = nn.MaxPool2d(kernel_size=2, ceil_mode=True) # 像素压缩
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2) #上采样  2倍
    ...
    def forward(self, x_copy, x, interpolate=True):
        ...
        # Concatenate
        x = torch.cat([x_copy, x], dim=1) # 论文中的左面和右面连接
        x = self.up_conv(x)
        return x
...
class UNet(BaseModel):
    def __init__(self, num_classes, in_channels=3, freeze_bn=False, **_):
        ...
        self.middle_conv = x2conv(1024, 1024) #原论文中没有

        self.up1 = decoder(1024, 512)
        self.up2 = decoder(512, 256)
        self.up3 = decoder(256, 128)
        self.up4 = decoder(128, 64)
        self.final_conv = nn.Conv2d(64, num_classes, kernel_size=1) # 映射到类别数目上
